...
namespace RDG
{
    ...
    public static class Vibration
    {
        ...
        /// <summary>
        /// Custom haptic "Swoosh" effect: short ascending vibration inspired by Heavy Click
        /// </summary>
        public static void VibrateSwoosh(bool cancel = false)
        {
            Initialize(); // Asegura inicialización

            if (isInitialized == false || HasVibrator() == false)
            {
                Debug.LogWarning("VibrateSwoosh(): Device has no vibrator or is not initialized");
                return;
            }

            if (cancel) Cancel();

            // Duraciones y amplitudes: 3 pulsos crecientes
            long[] pattern = { 0, 20, 15, 25, 15, 40 };       // tiempos: off/on/off/on...
            int[] amplitudes = { 0, 90, 0, 160, 0, 255 };     // intensidades ascendentes

            if (doesSupportVibrationEffect())
            {
                if (HasAmplitudeControl())
                {
                    vibrateEffect(pattern, amplitudes, -1);
                }
                else
                {
                    // Si no hay control de amplitud, hace un one-shot fuerte
                    vibrateEffect(80, 255);
                }
            }
            else
            {
                // Compatibilidad con versiones antiguas
                vibrateLegacy(pattern, -1);
            }
        }
    }
}